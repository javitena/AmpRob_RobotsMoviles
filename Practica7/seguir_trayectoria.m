function seguir_trayectoria (ini,fin,map_in,G,nodos)
close all
...
show(mapa);
hold on
[coste, ruta] = Dijkstra(G,ini,fin); 
for i=1:1:(length(ruta)-1)
    fin=campos_potenciales([nodos(ruta(i),2:3)],[nodos(ruta(i+1),2:3)],mapa);
    if fin==true %Si no puede llegar al destino sale del bucle y no sigue con el siguiente nodo
        break
    end
end
if fin==false
